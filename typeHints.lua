...
---@field StartRendering fun(self: Project, jobId1: integer, jobId2: integer): boolean Starts rendering jobs indicated by the input job ids.
...
---@field AddItemListToMediaPool fun(self: MediaStorage, item1: Item, item2: Item): Clip[] Adds specified file/folder paths from Media Storage into current Media Pool folder. Input is one or more file/folder paths. Returns a list of the MediaPoolItems created.
...
---@field CreateTimelineFromClips fun(self: MediaPool, name: string, clip1: Clip, clip2: Clip): Timeline Creates new timeline with specified name, and appends the specified MediaPoolItem objects.
...
---@field #Example:ImportMedia fun(self: MediaPool, "FilePath":"file_%03ddpx": string, "StartIndex":1: integer, "EndIndex":100: integer): boolean Example: ImportMedia([{"FilePath":"file_%03d.dpx", "StartIndex":1, "EndIndex":100}]) would import clip "file_[001-100].dpx". ExportMetadata(fileName, [clips]) --> Bool 
...
---@field ApplyGradeFromDRX fun(self: Timeline, path: string, gradeMode: GradeMode, item1: Item, item2: Item): boolean Loads a still from given file path (string) and applies grade to Timeline Items with gradeMode (int): 0 - "No keyframes", 1 - "Source Timecode aligned", 2 - "Start Frames aligned".
